...
The relative position of the scores in this range indicates subtle differences in translation quality. Based on the provided source, translation, and reasoning, output only the numerical score.""" 
    def create_user_message(source_text, mt_text, gpt_explanation): #<--- added gpt reasoning as a variant
...
{gpt_explanation}"""
    ...
    def tokenize(example):
        prompt_messages = [
            ...
            {"role": "user", "content": create_user_message(example['src'], example['mt'], example['gpt_explanation'])},
        ]
